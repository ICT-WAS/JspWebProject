...
>>>>>>> 51dd1424b061b9f4f6697effe481c757ea3e76ce
	...
	// 필터링된 리스트 + 총 카운트 가져오기
	public ProductPage getFilteredProductPage(Integer categoryId, String name, Double minPrice, Double maxPrice,
		...
		// ----- 리스트 가져오기 -------
		String sql = "SELECT * FROM ( " + " SELECT p.*, ROWNUM rnum " + " FROM ( "
				+ " SELECT * FROM PRODUCT WHERE 1=1 ";

		// 필터링 조건 추가
		sql += setConditionSql(categoryId, name, minPrice, maxPrice, brand);

		sql += " ORDER BY PRODUCT_ID DESC" + " ) p " + " WHERE ROWNUM <= ? " + // endRow
				") " + " WHERE rnum >= ?"; // startRow
	...
	private int setConditionPlaceHolder(PreparedStatement pstmt, int index, Integer categoryId, String name,
			Double minPrice, Double maxPrice, String brand) throws SQLException {
